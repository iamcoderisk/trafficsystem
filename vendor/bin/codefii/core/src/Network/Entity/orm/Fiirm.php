...
abstract class Fiirm extends Database
{
    ...
    public function relatedTo(array $values,string $table=null,
        string $joinType=null){
        if(count($values)==1){
            foreach($values as $key=>$value){
            $this->sql =" SELECT * FROM {$this->table} LEFT JOIN {$table} ON {$this->table}.{$key}={$table}.{$value} ORDER BY created DESC";
            if($this->stmt= $this->conn->prepare($this->sql)){
                return $this;
            }
            }
